...
import { Assets, assetURL, json, type Catalog, type LevelData,type SceneryMaterial } from './assets';
 ...
 */
/**
 * Collision for the buildings the world export left out, generated by
 * tools/building_collision.mjs from each building's own geometry.
 */
async function buildingCollision(level:number){
  try{
    const meta=await json<{positions:[number,number]}>(`collision/buildings-level${level}.json`);
    const response=await fetch(assetURL(`collision/buildings-level${level}.bin`));
    if(!response.ok)return undefined;
    const binary=await response.arrayBuffer();
    const geometry=new THREE.BufferGeometry();
    geometry.setAttribute('position',new THREE.BufferAttribute(new Float32Array(binary,meta.positions[0]*4,meta.positions[1]),3));
    return geometry;
  }catch{return undefined;}      // a level without the file simply keeps what it had
}
function withBuildings(bodies:THREE.BufferGeometry,buildings?:THREE.BufferGeometry){
  if(!buildings)return bodies;
  const merged=mergeGeometries([bodies,buildings]);
  bodies.dispose();buildings.dispose();
  return merged??bodies;
}
...
export class World {
  ...
  async load(level:number,progress:(value:number)=>void){
    ...
    const [surfaces,scenery,ground,physics,objects,navigation,buildings]=await Promise.all([json<Record<string,SceneryMaterial>>('remaster/scenery-materials.json'),json<{scenes:string[]}>('remaster/scenery.json'),json<Record<string,string>>('remaster/surfaces.json'),json<InteriorCollision>(`collision/world-level${level}.json`),json<WorldObjectsData>(`world/objects${level}.json`),json<RoadNavigation>(`world/navigation${level}.json`),buildingCollision(level)]);
    ...
    // Buildings the export left without a shape carry their own geometry as collision.
    // They join the static solids rather than the terrain, whose triangles are index-
    // locked to the per-triangle surface types.
    const bodies=withBuildings(staticCollisionGeometry({...physics,shapes}),buildings);this.terrain=new Terrain(collision,this.data,bodies,this.data.scenes.flatMap(scene=>objects.terrainTypes[scene]));bodies.dispose();this.exteriorTerrain=this.terrain;
