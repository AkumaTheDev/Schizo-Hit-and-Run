...
export function devTools(input:Input,callbacks:{resume:()=>void;objective:()=>void;retry:()=>void;scenario:(value:string)=>Promise<void>;state:()=>string;pursuit:()=>void;clearPursuit:()=>void;testCoins:()=>void}){
  ...
  const codes:Record<string,string>={W:'KeyW',A:'KeyA',S:'KeyS',D:'KeyD',E:'KeyE',F:'KeyF',B:'KeyB',C:'KeyC',SPACE:'Space',SHIFT:'ShiftLeft',H:'KeyH'};
  buttons[0].onclick=()=>{
